...
#ifndef FIFO_filter_top_fft_config_fwd_data_V_HH_
#define FIFO_filter_top_fft_config_fwd_data_V_HH_

#include <systemc>
using namespace std;
SC_MODULE(FIFO_filter_top_fft_config_fwd_data_V) {
    static const unsigned int DATA_WIDTH = 16;
    static const unsigned int ADDR_WIDTH = 12;
    static const unsigned int FIFO_filter_top_fft_config_fwd_data_V_depth = 2049;
    ...
    sc_core::sc_signal< sc_dt::sc_lv<DATA_WIDTH> >  mStorage[FIFO_filter_top_fft_config_fwd_data_V_depth];
    ...
    SC_CTOR(FIFO_filter_top_fft_config_fwd_data_V) : mTrace(0) {
        ...
        SC_METHOD(proc_dout);
        sensitive << mOutPtr;
        for (unsigned i = 0; i < FIFO_filter_top_fft_config_fwd_data_V_depth; i++) {
    ...
    ~FIFO_filter_top_fft_config_fwd_data_V() {
        ...
        if (reset.read() == sc_dt::SC_LOGIC_1) {
            ...
            if (if_read_ce.read() == sc_dt::SC_LOGIC_1
               ...
               && internal_empty_n.read() == sc_dt::SC_LOGIC_1) {
                ...
                if (mOutPtr.read().to_uint() == (FIFO_filter_top_fft_config_fwd_data_V_depth-1)) {
                    ptr = 0;
                ...
                assert(ptr.to_uint() < FIFO_filter_top_fft_config_fwd_data_V_depth);
                mOutPtr.write(ptr);
            }
            if (if_write_ce.read() == sc_dt::SC_LOGIC_1
                ...
                if (ptr.to_uint() == (FIFO_filter_top_fft_config_fwd_data_V_depth-1)) {
                    ptr = 0;
                    mFlag_nEF_hint.write(~mFlag_nEF_hint.read());
                } else {
                    ptr++;
    ...
    void proc_dout() {
        ...
        if (ptr.to_uint() > FIFO_filter_top_fft_config_fwd_data_V_depth) {
...
#endif //FIFO_filter_top_fft_config_fwd_data_V_HH_
